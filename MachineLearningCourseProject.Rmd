...
rcorr(as.matrix(pml_training_no_NA[-53]))          # Pearson correlations and p-values.
pca <- princomp(pml_training_no_NA[-53], cor=TRUE) # Principal components are uncorrelated.
pcadf <- as.data.frame(pca$scores)
sapply(pcadf, function(x) lillie.test(x)) # PCs are just as non-normal as the original data.
