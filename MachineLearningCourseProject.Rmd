...
This project uses data^[See [Human Activity Recognition](http://web.archive.org/web/20161224072740/http:/groupware.les.inf.puc-rio.br/har).] from accelerometers on the belt, forearm, arm, and dumbell of six participants, who were asked to perform barbell lifts correctly and incorrectly in five different ways. The goal is to predict the manner in which they did the exercise, using the dependent variable `classe` in the training dataset. 
...
```{r EDA}
glimpse(pml_training) # Gives us a concise look at all the variables.
...
sb <- pml_training[, 37:45] # How to carve out chunks of non-sucking variables.
# dplyr::select() lets y'all use variables names as positions! And series of frikkin' positions!
shitty <- select(sb, "accel_belt_y":"magnet_belt_x", "gyros_belt_x":"gyros_belt_z") 
# chart.Correlation(sb, method = "pearson", histogram = TRUE) # Slow, but pretty (and useful).
# pairs(sb, upper.panel = NULL) # Faster.
```
